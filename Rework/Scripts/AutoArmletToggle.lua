...
function Tick( tick )
	...
	local myHero = entityList:GetMyHero()

	local armlet = myHero:FindItem("item_armlet")
	...
	local armState = myHero:DoesHaveModifier("modifier_item_armlet_unholy_strength")

	if not myHero:DoesHaveModifier("modifier_ice_blast") and (ScriptConfig.active or ScriptConfig.manual) and armlet.cd == 0 and SleepCheck() then
		if myHero.health <= ScriptConfig.minhp + 0 or (ScriptConfig.manual and armState) then
			if armState then
				extraToggle = 2
			else
				extraToggle = 1
	...
	if extraToggle > 0 then
		if armlet.state == STATE_READY and armlet.cd == 0 then
			if myHero:SafeCastItem("item_armlet") then
